return function(ESP, Utils, Overrides)
    ...
    -- ── Apply colour + thickness + alpha + endpoints to a Line pair ───────────
    ...
    local function Update(E, Target)
        ...
        local Distance            = 0
        local InRange             = false
        -- Default IsAlive=true so targets appear even if GetHealth fails/errors
        local Health, MaxHealth   = 100, 100
        local IsAlive             = true
        local InEnemy, TeamColor  = true, WhiteColor
        ...
        if Character and RootPart then
            local ok, err = pcall(function()
                ...
                InRange             = IsWithinReach(
                    GetFlag(Flags, Flag, "/DistanceCheck"),
                    GetFlag(Flags, Flag, "/Distance"),
                    Distance)
                Health, MaxHealth, IsAlive = GetHealth(Target, Character, E.Mode)
                InEnemy, TeamColor         = GetTeam(Target, Character, E.Mode)
            end)
            if not ok then
                -- Silently skip this target this frame if something errored
                return
            end
        ...
        local Vis    = (Character ~= nil) and (RootPart ~= nil) and OnScreen  and InRange and IsAlive and TeamOK
        local ArrVis = (Character ~= nil) and (RootPart ~= nil) and (not OnScreen) and InRange and IsAlive and TeamOK
            ...
            if BoxOn then
                ...
                if NameOn or HpTxtOn or DistOn or WepOn then
                    ...
                    if HpTxtOn then
                        ...
                        TB.Health.Text         = tostring(Floor(HPct * 100)) .. "%"
                    ...
                    if DistOn then
                        ...
                        TB.Distance.Text         = tostring(Floor(Distance)) .. " studs"
